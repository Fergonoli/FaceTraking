...
namespace FaceTrackingBasics
{
    ...
    public partial class FaceTrackingViewer : UserControl, IDisposable
    {
        ...
        private Skeleton[] skeletonData;
         ...
         public List<Point> DevolverPuntos()
         {
             // Lista donde se almacenen los puntos que devuelve la máscara, preparados para la BBDD
             List<Point> res = new List<Point>();

             //Lista donde se almacenan los puntos
             List<Point> Puntos = new List<Point>();

 
             //Recorremos la lista de puntos de faceInformation, del esqueleto formado para la cara
             foreach (SkeletonFaceTracker faceInformation in this.trackedSkeletons.Values)
             {
                 //Se carga en la variable Puntos, los puntos de la mascara
                 Puntos = faceInformation.DevolverFacePoints();
                 ...
                 for (int i = 0; i < Puntos.Count; i++)
                 {
                     ...
                     float nX = (float)Puntos[i].X;
                     float nY = (float)Puntos[i].Y;

                     Point par = new Point(nX, nY);
                     res.Add(par);                                           
                 }
             }
         ...
         public List<Point3D>  DevolverPuntos3D()
         {
             // Lista donde se almacenen los puntos que devuelve la máscara, preparados para la BBDD
             ...
             {
                 //Se carga en la variable Puntos, los puntos de la mascara
                 Puntos = faceInformation.DevolverFacePoints3D();
                 ...
                 for (int i = 0; i < Puntos.Count; i++)
                 {
                     ...
                     float nX = (float)Puntos[i].X;
                     float nY = (float)Puntos[i].Y;
                     float nZ = (float)Puntos[i].Z;

                     Point3D trio = new Point3D(nX, nY, nZ);
                     res.Add(trio);
                 }
             }
        ...
        private class SkeletonFaceTracker : IDisposable
        {
            ...
            public EnumIndexableCollection<FeaturePoint, Vector3DF> facePoints3D;
            ...
            //Metodo creado para devolver los puntos faciales XY que se recogen de la Kinect 
            public List<Point> DevolverFacePoints()
            {
                List<Point> Puntos = new List<Point>();

                //Se recorre la lista de puntos faciales y se recojen sus coordenadas
                for (int i = 0; i < this.facePoints.Count; i++)
                {
               
                    Puntos.Add(new Point(this.facePoints[i].X + 0.5f, this.facePoints[i].Y + 0.5f));
                }
            ...
            //Metodo creado para devolver los puntos faciales XYZ que se recogen de la Kinect
            public List<Point3D> DevolverFacePoints3D()
            {
                List<Point3D> Puntos = new List<Point3D>();
                ...
                if (this.facePoints != null)
                {
                    ...
                    for (int i = 0; i < this.facePoints.Count; i++)
                    {
                        Puntos.Add(new Point3D(this.facePoints3D[i].X + 0.5f, this.facePoints3D[i].Y + 0.5f, this.facePoints3D[i].Z + 0.5f));
                    }
                }
